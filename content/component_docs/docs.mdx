...
Use ⌥+s (option+s) to save. This is required or all data will be lost. This is not done automatically to avoid a huge number of writes to the database that would increase operational cost.
...
- No special characters in field names. The `name` field can be any string so long as it does not contain special characters (characters other than letters, numbers and white-space).
  ...
  - Make it as descriptive as you like. This will be displayed along with the results, but is visible only to administrators.
- Line breaks are only allowed in jsx strings started with a backtick quotation (top left of keyboard). 
- Copy and paste the components as needed, and just modify their properties. The documentation of each component here is more of a reference than a guide, and can read on an as-needed basis.
...
Here, the `past` and `future` keys, if set to true will collect dates for 10 years in the past or future respectively.
